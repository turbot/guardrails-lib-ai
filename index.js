...
class AI {
  constructor(config = {}) {
    ...
    if (!config.provider) {
      throw errors.badConfiguration("Provider is required. Supported providers: openai, anthropic, azure");
    }
    ...
    this.defaultConfig = {
      ...
      endpoint: config.azureOpenAiEndpoint, // Azure endpoint
      deploymentName: config.azureOpenAiDeploymentName, // Azure deployment name
      apiVersion: config.azureOpenAiApiVersion, // Azure API version
    };
    ...
    // Initialize Azure OpenAI client using AzureOpenAI from openai package
    if (normalizedProvider === "azure openai") {
      ...
      this.azureOpenAI = new AzureOpenAI({
        ...
        apiVersion: config.azureOpenAiApiVersion || "2024-04-01-preview",
      });
      this.azureDeploymentName = config.azureOpenAiDeploymentName;
    }

    this.callAzureOpenAI = async (prompt, options = {}) => {
      try {
        const messages = [];
        ...
        const requestOptions = {
          messages,
          max_tokens: options.max_tokens || this.defaultConfig.max_tokens || 4096,
          temperature: options.temperature !== undefined ? options.temperature : this.defaultConfig.temperature || 1,
          top_p: 1,
          model: this.azureDeploymentName,
        };

        const completion = await this.azureOpenAI.chat.completions.create(requestOptions);
        if (completion?.error !== undefined && completion.status !== "200") {
  ...
  async callOpenAI(prompt, options = {}) {
    try {
      ...
      if (!modelIsGPT5 && (options.temperature !== undefined || this.defaultConfig.temperature !== undefined)) {
        requestOptions.temperature =
          options.temperature !== undefined ? options.temperature : this.defaultConfig.temperature;
      }
